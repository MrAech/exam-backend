...
public class OAuth2SuccessHandler implements AuthenticationSuccessHandler {
  ...
  public void onAuthenticationSuccess(HttpServletRequest req, HttpServletResponse res, Authentication authentication)
    ...
    if (!"google".equals(registrationId)) {
      throw new RuntimeException("Only Google OAuth is enabled"); // For now...well....
    }
    ...
    RefreshToken refTknObj = RefreshToken.builder()
        .jti(jti)
        .user(user)
        .revoked(false)
    ...
    refreshTokenRepo.save(refTknObj);

    String refreshToken = jwtService.generateRefreshToken(user, refTknObj.getJti());
    cookieService.attachRefreshCookie(res, refreshToken, (int) jwtService.getRefreshTTLSec());

    res.sendRedirect(frontEndSuccessUrl);
  }
